...
- [x] **St4=外装と地形の質感・遠景・市街戦マップ**(人間要望 2026-07-31「ロボパーツや地形のテクスチャが
  ...
  **リリース時に REPLAY_V 4→5 が必要**(fields.js が凍結トリオに入っているため。掟2・3)。
  ...
  - **参考(未対応・次セッション以降の候補)**: ①初回フレームのテクスチャ生成コスト(純JSだけで76ms、
    ...
    ③`harness-budget.mjs:19` だけ戦場をハードコードのまま(今は本番/dev 双方に在る5戦場なので嘘の
    カバレッジは出ていないが、ibara/shigai を総当たりで一度も回さない+対象と戦場一覧を出力しないので
    将来の id 変更で黙って plain へ落ちる。`harness.mjs` と同じ `FIELDS.map(f=>f.id)` 化が筋)。
  ...
  - **昇格手順**(人間承認後): ⑴ 描画側 `tex.js`(新規)・`r3d.js`・`r3d-three.js`・`game.js`(DEV=false 戻し)を
    public/ へ手動コピー ⑵ `node tools/harness/release-sim.mjs` で fields.js を本番へ入れ **REPLAY_V 4→5**+
    `sims/v5/` 凍結 ⑶ `replay.js` の FIELD_CODES 末尾へ `'shigai'` 追記(掟2。並べ替え禁止)
...
- [ ] **小障害物の当たり判定=地形に表情を**(人間要望 2026-07-31・**次テーマ**): St4 の装飾層は当たり判定ゼロで、
  街灯/信号/電柱の支柱33本(高さ15〜21=機体全高の3.5〜5倍。機体は実測4.24ワールド単位・最大5.10)と
  廃車6台が機体可動域内にあるため機体がすり抜ける
  (実測40戦20,193tickで街灯類22/40試合・廃車15/40試合でめり込み。**St4 では据え置きと人間判断 2026-07-31**)。
  人間の設計方針: ①市街の縁石・瓦礫に当たり判定を付ける ②乗り越えるための移動/回避ペナルティ
  ③**乗り越え中は当たり判定が変わる**(丘の頂点=露出が大きい、等) ④他の戦場にも小障害物を置いて表情を出す。
  **シム改修=REPLAY_V が上がる**ので St4 の昇格(4→5)に同梱するか別リリースにするかを最初に決める。
  高さの概念をシムに入れる話に触るため、`obstacles` の型拡張(kind 追加 or 高さ/踏破コストの属性追加)と
  ハザードAI(踏む意欲×踏む価値)への織り込み方が設計の芯。ハーネス/hazard-probe に踏破の判定ゲートが要る。
