...
import { createR3D, mechMesh, mechFocus } from './r3d.js';
...
export const RANK_FIELDS = { E: ['sekichu','deitan','crater'], D: ['haikyo','sekichu','deitan'],
  C: ['sekichu','deitan','crater'], B: ['crater','sekichu','haikyo'],
  A: ['deitan','haikyo','crater'], S: ['sekichu','crater','haikyo'] };
...
const DEV = false;          // kouki-dev(開発版)生成時に make-dev.mjs が true へ書き換える。
...
// St2(第1段=二脚): 観戦3DをThree.jsへ移行中。既定=Three、?soft=1 で旧ソフトウェア版(A/B比較)。
// Three初期化(WebGL)に失敗したら自動でソフトウェア版へフォールバック=常にgreen。
const R3D_SOFT = Q.has('soft');
function makeR3D(canvas) {
  if (!R3D_SOFT) { try { return createR3DThree(canvas); } catch (e) { console.warn('[r3d] Three init 失敗→ソフトウェア版へ', e); } }
  return createR3D(canvas);
}
