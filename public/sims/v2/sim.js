...
function pAcc(w, dist, shooter, slot) {
  ...
  return clamp(w.acc * bm.acc + shooter.st.aimBonus + (shooter.pilotAcc || 0) + ARM_ACC[shooter.pd[slot]],
               0.05, 0.97);
}
// 回避(目標側)ロール確率 — 今の移動速度でスケール(×0.6スケール後の基準36m/s)+脚損傷
...
export function simulate(buildA, buildB, seed, opts = {}) {
  ...
  while (t < TMAX && m[0].alive && m[1].alive) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      for (let s2 = 0; s2 < me.wpns.length; s2++) {
        ...
        const group = { t, si: i, wname: w.name, kind: w.kind, dist,
          nHit: 0, dmgSum: 0, nDodge: 0, nParry: 0, nMiss: 0, remain: 0, n: rounds };
        groups.push(group);
        ...
        for (let j = 0; j < rounds; j++) {
          ...
          // 3段ロール: 外れ → 回避 → パリィ
          let outcome = 'hit';
          if (rng() > pAcc(w, dist, me, s2)) outcome = 'miss';
          else if (rng() < pDodge(foeM)) outcome = 'dodge';
          else if (rng() < pParry(w, foeM)) { outcome = 'parry'; foeM.en = Math.max(0, foeM.en - 6); }
          // ダメージ(帯域倍率×装甲。ドリルは装甲貫通=defenseの40%を無視)
    ...
    while (pending.length && pending[0].tImpact <= t + DT) {
      ...
      if (p.outcome === 'hit') {
        ...
        if (p.outcome === 'dodge') { g.nDodge++; ev(p.tImpact, 'dodge', { who: p.si, targ: p.ti, wpn: p.w.kind, wname: p.w.name, x: tgt.x, y: tgt.y }); }
        else if (p.outcome === 'parry') { g.nParry++; ev(p.tImpact, 'parry', { who: p.si, targ: p.ti, wpn: p.w.kind, wname: p.w.name, x: tgt.x, y: tgt.y }); }
  ...
  ev(duration, 'end', { winner, reason: endReason,
    hpA: Math.max(0, Math.round(m[0].hp)), hpB: Math.max(0, Math.round(m[1].hp)) });
...
function buildLog(events, groups, names, field) {
  ...
  for (const e of events) {
    if (e.kind === 'fire') {
      ...
      if (g.nDodge) parts.push(`${g.nDodge}発回避される`);
